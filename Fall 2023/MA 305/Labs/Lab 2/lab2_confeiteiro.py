...
def approx_sqrt(start=None, precision=1e-7, return_list=False):
    ...
    return (x_0, vals_len, vals) if return_list else (x_0, vals_len)


# read user input (w/ validation loop)
while True:
    try:
        input_val = float(input(">> Enter number for square root approximation: "))
        break
    except:
        print("Invalid Input.\n")
...
print(f">> Total num. iterations: {count} iterations")
