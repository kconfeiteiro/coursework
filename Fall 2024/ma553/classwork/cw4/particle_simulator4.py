...
class ParticleSimulator:
    ...
    def evolve_openmp(self, tmax, dt=0.00001):
        nsteps = int(tmax / dt)

        r_i = np.array([[p.x, p.y] for p in self.particles])
        ang_speed_i = np.array([p.ang_speed for p in self.particles])

        c_evolve_openmp(r_i, ang_speed_i, dt, nsteps)
...
def benchmark(n=100, method='python'):
    particles = [
        Particle(uniform(-1.0, 1.0), uniform(-1.0, 1.0), uniform(-1.0, 1.0)) for i in range(n)
    ]
    ...
    if method == 'openmp':
        simulator.evolve_openmp(0.1)

def benchmark_memory(n=1000, method='python'):
    particles = [
        Particle(uniform(-1.0, 1.0), uniform(-1.0, 1.0), uniform(-1.0, 1.0)) for i in range(100000)
    ]
    simulator = ParticleSimulator(particles)
    if method == 'python':
