...
%% download data
[~,~,rawData] = xlsread('FarmersMarketData.xlsx');
...
while isnan(x2) || x2<-180 || x2>0
    x2 = str2double(input('Error, re-enter the longitude: '));
end
y2 = str2double(input('Enter the latitude: '));
while isnan(y2) || y2>90 || y2<0
...
rowFound = find(min(distance)==distance);

%output nearest famer(s)
fprintf('\n\n--------------------------------------------------------\n');
fprintf('Name: %s \nAddress: %s \nTime(s): %s',names(rowFound,1),addresses(rowFound,:),mHours(rowFound,1));
